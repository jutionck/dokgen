...
import { fmtDate, fmtDateLong } from "@/components/documents/templates/shared";
import {
  pdfStyles,
  PdfClient,
  PdfItemsTable,
...
export function PenawaranPdf({ data }: { data: TemplateData }) {
  ...
  return (
    <PdfDocumentShell data={data}>
      ...
      <Text style={pdfStyles.sectionTitle}>Rincian Biaya Penawaran</Text>
...
export function QuotationPdf({ data }: { data: TemplateData }) {
  const { company, doc, client } = data;
  const extra = doc.extra;
  const validityDays = Number(extra.validity_days) || 14;
  return (
...
export function InvoicePdf({ data }: { data: TemplateData }) {
...
export function BastPdf({ data }: { data: TemplateData }) {
  const { company, doc, client } = data;
  const extra = doc.extra;
  return (
    <PdfDocumentShell data={data}>
...
export function KontrakPdf({ data }: { data: TemplateData }) {
  const { company, doc, client } = data;
  const extra = doc.extra;
...
export function PdfRenderer({ data }: { data: TemplateData }) {
  switch (data.doc.type) {
    case "penawaran":
      return <PenawaranPdf data={data} />;
    case "quotation":
      return <QuotationPdf data={data} />;
    case "invoice":
      return <InvoicePdf data={data} />;
    case "bast":
      return <BastPdf data={data} />;
    case "kontrak":
      return <KontrakPdf data={data} />;
    default:
      return <PdfDocumentShell data={data} />;
  }
}